...
def cmd_ingest(args):
    """Download datasets as specified in config."""
    from src.data.data_ingestion import download_datasets
    config = load_config(args.config)
    download_datasets(config, force=args.force)


def cmd_pretrain(args):
    """Run pre‑training on WikiText‑103 (MLM)."""
...
def cmd_finetune(args):
    """Run fine‑tuning on FUNSD (token classification)."""
    from src.models.rifd import RIFD
    from src.tokenizer_utils import load_tokenizer
    ...
    # Use test set as validation (FUNSD has no public val)
    ...
    # Load pre‑trained encoder weights if available (optional)
    if args.pretrained_checkpoint:
        ckpt = Path(args.pretrained_checkpoint)
        if ckpt.exists():
            logger.info(f"Loading pre‑trained encoder from {ckpt}")
            state = torch.load(ckpt, map_location="cpu")
            # Filter out NER head keys (shouldn't exist, but safety)
